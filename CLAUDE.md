...
All visitor-facing copy (essays, cards, deks, TL;DRs, UI strings) must read as written by a
human, not generated by AI. In particular:

- **No em dashes (`—`).** Use a period, comma, colon, or parentheses instead. Rewrite the
  sentence rather than substitute an en dash or hyphen as a stand-in.
- Avoid other AI writing tells: rule-of-three lists dropped into prose, "it's not just X,
  it's Y" constructions, throat-clearing openers ("In today's world...", "It's worth
  noting..."), and over-hedged conclusions. Write the plain sentence instead.
- Vary sentence length and structure the way a person editing their own writing would;
  don't default to uniform, evenly-weighted clauses.
- This applies to everything under `src/data/` and `src/content/essays/`, including content
  supplied by Lee for adaptation into the site's format — check it for these tells too, not
  just content drafted from scratch.
